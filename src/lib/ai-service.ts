...
【重要提示 - 内容完整性】
- 必须生成完整的文章内容，包括所有必需的部分
- 不要因为任何原因截断内容，必须生成到文章结尾
- 确保文章字数达到1500-2500字的要求
- 如果内容较长，请继续生成直到完成所有部分
...
1. **核心逻辑：**（橙色加粗 #ff7d00）
   - 【格式要求】必须使用 `**核心逻辑：**` 格式，不能省略冒号
   - 【格式要求】必须使用双星号加粗，格式为：`**核心逻辑：**`（注意冒号后有一个空格）
...
### （3）文末总结框【必须生成 - 不能省略】
- **【强制要求】** 这是文章的结尾部分，必须生成，不能省略或截断
- **【强制要求】** 必须包含"核心逻辑总结"和"行动呼吁"两个部分
- **背景色**：#f0e6ff（浅紫）
- **内边距**：20px
- **内容格式**：
...
【格式检查 - 核心逻辑】
- ✅ "核心逻辑："是否使用 `**核心逻辑：**` 格式（双星号加粗，包含冒号和空格）
- ✅ "核心逻辑："是否在冒号后有一个空格
- ✅ 所有小标题（核心逻辑、为什么重要、如何实施、真实案例、入门门槛）是否都使用 `**标题名：**` 格式

【完整性检查 - 结尾部分】
- ✅ 文章结尾是否包含"文末总结框"（这是必须生成的部分，不能省略）
- ✅ 文末总结框是否包含"核心逻辑总结："部分
- ✅ 文末总结框是否包含"行动呼吁："部分
- ✅ 文末总结框是否使用 > 引用块格式（浅紫背景 #f0e6ff）
- ✅ 结尾部分是否完整，没有被截断
...
【格式要求 - 必须严格遵守】
1. "核心逻辑："必须使用 **核心逻辑：** 格式（双星号加粗，冒号后有一个空格）
2. 所有小标题必须使用 **标题名：** 格式
3. 文章结尾必须包含"文末总结框"，包含"核心逻辑总结"和"行动呼吁"两个部分
4. 文章不能在没有生成完整结尾的情况下结束

【内容完整性要求 - 非常重要】
1. 必须生成完整的文章内容，包括所有5个案例模块和文末总结框
2. 不要因为长度限制而截断内容，必须生成到文章结尾
3. 如果内容较长，请继续生成直到完成所有必需的部分
4. 确保文章字数达到1500-2500字的要求
...
${formatSection}`;
  ...
  const messages: LLMMessage[] = [
    ...
    })),
    {
      ...
      content: integratedData 
        ? `${prompt}\n\n【重要要求】\n1. 请基于上述系统提示中提供的「整合后的资料」生成文章，确保文章内容与资料高度相关，不要偏离主题。\n2. 必须使用 \`**核心逻辑：**\` 格式（双星号加粗，冒号后有一个空格）。\n3. 必须生成完整的结尾部分，包括"核心逻辑总结"和"行动呼吁"，不能省略或截断。` 
        : `${prompt}\n\n【格式要求】\n1. 必须使用 \`**核心逻辑：**\` 格式（双星号加粗，冒号后有一个空格）。\n2. 必须生成完整的结尾部分，包括"核心逻辑总结"和"行动呼吁"，不能省略或截断。`,
    },
  ];
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // 根据内容类型设置 max_tokens
      // 文章类型需要更多 tokens 来生成完整内容（1500-2500字）
      if (contentType === 'article') {
        // 使用更大的 max_tokens 确保能生成完整的文章
        // 6000 tokens 可以支持约 3000-4000 字的内容（包括提示词消耗）
        invokeOptions.max_tokens = 6000;
      } else {
        // 其他内容类型使用默认值
        invokeOptions.max_tokens = 2000;
      }
